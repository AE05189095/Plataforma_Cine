...
// Registrar log de inicio de sesión
    ...
    // Poner cookie HttpOnly con el token (para peticiones con credentials: 'include')
    try {
      const cookieOptions = {
        httpOnly: true,
      ...
      console.warn('No se pudo setear cookie de sesión:', cookieErr && cookieErr.message ? cookieErr.message : cookieErr);
    }

    // Enviar respuesta (también devolvemos el token en JSON por compatibilidad)
    return res.json({
      token,
...
    );
    try {
      ...
      console.warn('No se pudo setear cookie de sesión (loginByModel):', cookieErr && cookieErr.message ? cookieErr.message : cookieErr);
    }
